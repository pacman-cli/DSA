 ...
 * This program implements the greedy approach to solve the fractional knapsack problem.
 * Given a set of items with weights and values, and a knapsack capacity W,
 * it finds the maximum value that can be achieved by taking fractions of items.
...
bool comp(Item i1, Item i2) {
    return i1.v * 1.0 / i1.w > i2.v * 1.0 / i2.w;  // Convert to double to avoid integer division
}
...
double FractionalKnapsack(vector<Item> items, int W) {
    ...
    for (int i = 0; i < items.size(); i++) {
        if (cap_left <= 0) break;  // Stop if knapsack is full
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        items.push_back({i + 1, w, v});  // Add item to vector with 1-based ID
    }
